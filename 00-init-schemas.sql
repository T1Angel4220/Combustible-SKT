...
DO $$ 
BEGIN
    -- Crear tipos de enumeración si no existen
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'rol_usuario_enum') THEN
        CREATE TYPE common_schema.rol_usuario_enum AS ENUM ('ADMIN', 'OPERADOR', 'SUPERVISOR', 'CONDUCTOR');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'estado_operativo_enum') THEN
        CREATE TYPE common_schema.estado_operativo_enum AS ENUM ('ACTIVO', 'MANTENIMIENTO', 'FUERA_SERVICIO', 'DISPONIBLE', 'EN_USO', 'RESERVADO');
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'tipo_maquinaria_enum') THEN
        CREATE TYPE common_schema.tipo_maquinaria_enum AS ENUM ('CAMION', 'VOLQUETE', 'EXCAVADORA', 'CARGADOR', 'GRUA', 'MOTONIVELADORA');
    END IF;
END $$;
